...
def fake_feedly_data():
    """Load mock alert data from a JSON file."""
    with open('tests/unit/fake_feedly_data.json', 'r', encoding='utf-8') as file:
...
def load_env_vars(monkeypatch):
    ...
    dotenv_path = 'tests/unit/.env.test'
    if not os.path.exists(dotenv_path):
        raise FileNotFoundError(f".env.test file not found: {dotenv_path}")
    env_vars = dotenv_values('tests/unit/.env.test')

    for k,v in env_vars.items():
        monkeypatch.setenv(k, v)

@pytest.fixture(scope="function") # NOTE: Depends on load_env_vars, which depends on monkeypatch, which is function-scoped. So fake_config_manager must be function-scoped, even though this is less efficient. Pytest does not allow a fixture with a broader scope to depend on a fixture with a narrower scope.
def fake_config_manager(load_env_vars):
    """
